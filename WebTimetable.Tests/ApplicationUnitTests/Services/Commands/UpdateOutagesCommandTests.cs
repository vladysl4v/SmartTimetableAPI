...
public class UpdateOutagesCommandTests
{
    ...
    public async Task UpdateOutagesCommand_Execute_SuccessfullyFills()
    {
        ...
        var mockOutagesRepo = new Mock<IRepository<OutageEntity>>();
        var mockUsersRepo = new Mock<IRepository<UserEntity>>();
        mockOutagesRepo.Setup(x => x.SaveChangesAsync(It.IsAny<CancellationToken>())).Verifiable();
        ...
        mockOutagesRepo.Verify(x => x.SaveChangesAsync(It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    public async Task UpdateOutagesCommand_Execute_EmptyResponse()
    {
        ...
        mockOutagesRepo.Verify(x => x.SaveChangesAsync(It.IsAny<CancellationToken>()), Times.Never);
    }
    ...
    public async Task UpdateOutagesCommand_Execute_CantFillDatabase()
    {
        ...
        var mockUsersRepo = new Mock<IRepository<UserEntity>>();
        var mockOutagesRepo = new Mock<IRepository<OutageEntity>>();
        mockOutagesRepo.Setup(x => x.RemoveRange(It.IsAny<IEnumerable<OutageEntity>>())).Throws<Exception>();
        mockOutagesRepo.Setup(x => x.SaveChangesAsync(It.IsAny<CancellationToken>())).Verifiable();
        var mockHttpFactory = new MockHttpFactory().Setup(data);
        var mockLogger = new Mock<ILogger<UpdateOutagesCommand>>();
        var command = new UpdateOutagesCommand(mockOutagesRepo.Object, mockUsersRepo.Object, mockHttpFactory, mockLogger.Object);
